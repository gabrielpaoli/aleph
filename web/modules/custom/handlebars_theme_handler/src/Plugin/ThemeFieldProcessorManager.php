...
class ThemeFieldProcessorManager extends DefaultPluginManager {
  ...
  public function getFieldData($field_list, $options = []) {
    ...
    if (isset($field_list['#items']) && $field_list['#items'] instanceof FieldItemListInterface) {
      ...
      $plugin_id = $this->getProcessor($field_list['#items'], $options);
   ...
   */
  public function getProcessor(FieldItemListInterface $field_list, ?array $options = []) {
    ...
    $variant = isset($options['variant']) ? $options['variant'] : NULL;
    ...
    $processor_id = $map[$key] ?? $map[$field_type];
    ...
    if (!isset($this->plugin_map)) {
      ...
      foreach ($plugins as $plugin_id => $plugin) {
        foreach ($plugin['field_types'] as $type) {
          ...
          $key = isset($plugin['variant']) ? implode('.', [
            $type,
            $plugin['variant']
          ]) : $type;
