...
class GenerateParagraphsCommand extends Command {
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    $templatePath = \Drupal::service('extension.path.resolver')->getPath('theme', $defaultTheme) . '/templates/block';
    ...
    $moduleComponentsPath = \Drupal::service('extension.path.resolver')
      ->getPath('module', $module) . '/src/Plugin/ThemeEntityProcessor/ParagraphsBlock';
    ...
    foreach ($templateDirectories as $templateDirectory) {
      if ($templateDirectory != $templatePath) {
        ...
        if (!empty($templatePathArray)) {
          ...
          // Generate Class name.
          $componentClassName = 'ParagraphsBlock' .
            $this->filesUtility->dashesToCamelCase($componentTemplateName, TRUE);
